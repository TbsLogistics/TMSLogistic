...
class DetailsPendingWorkingController extends GetxController {
  ...
  void postSetRuningTypeFull({required String handlingId}) async {
    ...
    var url =
        "${AppConstants.urlBaseTms}/api/Mobile/ChangeStatusHandling?id=$handlingId&maChuyen=${idChuyen.value}";
    try {
  ...
  void postSetRuning({required String maChuyen}) async {
    ...
    var url =
        "${AppConstants.urlBaseTms}/api/Mobile/SetRuningLessType?maChuyen=$maChuyen";
    try {
      response = await dio.post(
        url,
        options: Options(
